...
const logoutUser = asyncHandler(async (req, res) => {
  ...
  return res
    ...
    .clearCookie("refreshToken2", options)
  ...
  if (currentUser?.avatar) {
    try {
      ...
      if (publicId) {
        ...
        console.log("Could not extract publicId from URL:", currentUser.avatar);
      }
    } catch (error) {
      // Log error but don't fail the request
      console.error("Failed to delete old avatar (non-fatal):", error.message);
    }
  }

  return res
    .status(200)
    .json(new ApiRes(200, user, "avatar upadated Successfully"));
});
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiRes(200, user, "Cover image upadated Successfully"));
});
